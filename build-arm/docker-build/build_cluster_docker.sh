...
package() {
        ...
        if [ "$(uname)" == "Darwin" ];then
          sed -i "" 's#mvn clean package -DskipTests#docker run --rm -u $(id -u):$(id -g) -v ${source_dir}/fateboard:/data/projects/fate/fateboard --entrypoint="" maven:3.8-openjdk-8 /bin/bash -c "cd /data/projects/fate/fateboard \&\& mvn clean package -DskipTests"#g' ../package-build/build_docker.sh
          sed -i "" 's#sh ./auto-packaging.sh#docker run --rm -u $(id -u):$(id -g) -v ${source_dir}/eggroll:/data/projects/fate/eggroll --entrypoint="" maven:3.8-openjdk-8 /bin/bash -c "cd /data/projects/fate/eggroll/deploy \&\& bash auto-packaging.sh"#g' ../package-build/build_docker.sh
        else
          sed -i 's#mvn clean package -DskipTests#docker run --rm -u $(id -u):$(id -g) -v ${source_dir}/fateboard:/data/projects/fate/fateboard --entrypoint="" maven:3.8-openjdk-8 /bin/bash -c "cd /data/projects/fate/fateboard \&\& mvn clean package -DskipTests"#g' ../package-build/build_docker.sh
          sed -i 's#sh ./auto-packaging.sh#docker run --rm -u $(id -u):$(id -g) -v ${source_dir}/eggroll:/data/projects/fate/eggroll --entrypoint="" maven:3.8-openjdk-8 /bin/bash -c "cd /data/projects/fate/eggroll/deploy \&\& bash auto-packaging.sh"#g' ../package-build/build_docker.sh
        fi
        ...
        cp ${WORKING_DIR}/../requirements.txt ${WORKING_DIR}/catch/
}

buildArmUbuntu() {
        echo "START BUILDING ARM UBUNTU PYTHON38 IMAGE"
        cd ${WORKING_DIR}/docker/python-38-ubuntu
        docker build --tag cjf/python-38-ubuntu:1.0 .
        echo "FINISH BUILDING ARM UBUNTU PYTHON38 IMAGE"
        cd ${WORKING_DIR}
}
